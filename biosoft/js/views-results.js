/* BIOsoft — Vistas: Captura, validación y corrección de resultados */
(function () {
  "use strict";
  ...
  function renderBandeja(root) {
    ...
    function build() {
      ...
      root.innerHTML =
        ...
        '<select id="f-estado"><option value="todos">Todos los estados</option><option value="pendiente">Pendientes</option><option value="preliminar">Preliminares</option><option value="validado">Validados</option></select>' +
  ...
  function renderCaptura(root, orderId) {
    ...
    function buildExamCard(ex, idx) {
      ...
      var rowsHtml = exCat.parametros.map(function (p) {
        ...
        var inputHtml;
        if (p.tipo === "numerico") {
          inputHtml = '<input type="number" step="any" data-param="' + p.codigo + '" value="' + U.esc(val) + '" ' + ((!editable || locked) ? "disabled" : "") + "/>";
        } else if (p.tipo === "cualitativo") {
          inputHtml = '<select data-param="' + p.codigo + '" ' + ((!editable || locked) ? "disabled" : "") + '><option value="">— Seleccionar —</option>' +
            p.opciones.map(function (o) { return '<option ' + (o === val ? "selected" : "") + ">" + o + "</option>"; }).join("") + "</select>";
        } else {
          inputHtml = '<textarea data-param="' + p.codigo + '" ' + ((!editable || locked) ? "disabled" : "") + ">" + U.esc(val) + "</textarea>";
        }
      ...
      card.innerHTML =
        '<div class="card-header"><div><h3 class="card-title">' + U.esc(exCat.nombre) + '</h3>' +
        '<span class="text-muted" style="font-size:12px">' + C.seccionNombre(ex.seccion) + " · CUPS " + exCat.cups + " · Muestra: " + U.esc(exCat.muestra) + "</span></div>" +
        ...
        '<div class="field" style="margin-top:12px"><label>Observaciones del examen</label><textarea data-obs ' + ((!editable || locked) ? "disabled" : "") + ">" + U.esc(ex.observaciones || "") + "</textarea></div>" +
        ...
        (editable && !locked ? '<button class="btn btn-outline btn-sm" data-action="borrador">Guardar borrador</button>' : "") +
        (editable && !locked ? '<button class="btn btn-outline btn-sm" data-action="preliminar">Guardar como preliminar</button>' : "") +
        (editable && !locked ? '<button class="btn btn-primary btn-sm" data-action="validar">' + U.icon("check") + " Validar y Firmar</button>" : "") +
        (editable && locked ? '<button class="btn btn-danger btn-sm" data-action="corregir">' + U.icon("lock") + " Corregir (requiere clave admin)</button>" : "") +
        "</div>";
      ...
      if (btnValidar) btnValidar.addEventListener("click", function () {
        ...
        confirmValidation(function () {
          ...
          ex.estado = "validado"; ex.validadoPor = session.nombre; ex.fechaValidacion = S.nowISO();
      ...
      var btnCorregir = card.querySelector('[data-action="corregir"]');
      if (btnCorregir) btnCorregir.addEventListener("click", function () { abrirCorreccion(ex, exCat, order, build); });
    ...
    function confirmValidation(onOk) {
      var wrap = U.openModal(
        ...
        '<p class="text-muted">Al validar, usted certifica con su usuario y clave (' + U.esc(BIO_AUTH.getSession().nombre) + ') que revisó y aprueba estos resultados. Esta acción queda registrada con fecha y hora en la trazabilidad.</p>' +
  ...
  function abrirCorreccion(ex, exCat, order, onDone) {
    var session = BIO_AUTH.getSession();
    var wrap = U.openModal(
      '<h3 class="modal-title">' + U.icon("lock") + " Corrección de Resultado Validado</h3>" +
      '<p class="text-muted">El resultado <b>' + U.esc(exCat.nombre) + "</b> ya fue validado. Para modificarlo se requiere la clave de administrador del laboratorio y el motivo de la corrección, dejando trazabilidad completa (usuario, fecha y hora).</p>" +
      '<div class="field"><label>Clave de administrador *</label><input type="password" id="c-clave"/></div>' +
      '<div class="field"><label>Motivo de la corrección *</label><textarea id="c-motivo" placeholder="Ej: Error de digitación en el valor de glucosa, se corrige según repetición de la prueba."></textarea></div>' +
    ...
    wrap.querySelector("#c-continuar").addEventListener("click", function () {
      ...
      U.closeModal(wrap);
      abrirEdicionCorreccion(ex, exCat, order, motivo, onDone);
    });
  }
